...
def plot_W2_curve(results, title_suffix=""):
    ...
    # ------------------------------------------------------------
# Standalone single-network training (train one net at a time,
# instead of the paired 2-layer/3-layer loop inside run_W2_experiment)
# ------------------------------------------------------------

def train_network(net, X_data, t_0, T, d, n_iters, batch_size, lr, device, print_every=500):
    """
    Train a single ScoreNet or ScoreNet3 instance via denoising score matching.

    Returns (net, loss_history) so the training curve can be inspected
    separately from sampling and evaluation.
    """
    net.to(device)
...
def plot_loss_curve(loss_history, title_suffix=""):
    """Plot training loss vs iteration for a single train_network run."""
...
# ------------------------------------------------------------
# Wasserstein-2 distance between two fixed sample sets
# (factored out of estimate_W2 so it can be called on its own)
# ------------------------------------------------------------

def wasserstein2_distance(samples_a, samples_b):
    """
    Compute the empirical W_2 distance between two equally-sized sample sets
    via exact discrete optimal transport (ot.emd2).
    """
    N = samples_a.shape[0]
...
# ------------------------------------------------------------
# True vs generated sample comparison plot
# ------------------------------------------------------------

def plot_true_vs_generated(X_true, samples_generated, d, title_suffix=""):
    """
    Overlaid per-dimension histograms comparing true data samples
    against samples generated by a trained network.
    """
    fig, axes = plt.subplots(1, d, figsize=(5 * d, 4))
    if d == 1:
        axes = [axes]
    for i in range(d):
        axes[i].hist(X_true[:, i], bins=100, alpha=0.6, label="true", density=True)
        axes[i].hist(samples_generated[:, i], bins=100, alpha=0.6, label="generated", density=True)
        axes[i].set_title(f"Dimension {i}")
        axes[i].legend()
    plt.suptitle("True vs generated samples " + title_suffix)
    plt.tight_layout()
    plt.show()