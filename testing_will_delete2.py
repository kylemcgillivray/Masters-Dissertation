...
iteration counts, to separate under-training from a genuine barrier.

The training loss cannot answer this on its own. With kappa(t) = sigma_t^2
the objective decomposes as
    E|z + sigma_t s|^2 = (irreducible)  +  sigma_t^2 E|s - grad log p_t|^2,
so score error is weighted by sigma_t^2, which is 0.002 at t = 1e-3. Errors
at small t are therefore nearly invisible to the loss while being exactly
what determines sample quality. The unweighted score_error_by_t recorded at
each checkpoint is the quantity that actually tracks progress.
